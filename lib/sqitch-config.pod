...
allow only alphanumeric characters and C<_>, and must start with an alphabetic
...
for newline character (LF), C<\t> for horizontal tabulation (HT, TAB) and
...
to silence status messages and to 2 or 3 to increase verbosity. Error
